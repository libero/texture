...
export default class AddSupplementaryFileWorkflow extends Component {
  ...
  render($$) {
    ...
    const title = $$('div')
      .addClass('se-title')
      .append(this.getLabel('supplementary-file-workflow-title'));
    ...
    const urlInput = $$(InputWithButton, {
      ...
      button: $$(Button)
        .append(this.getLabel('add-action'))
        .on('click', this._onExternalFileImport),
    });

    el.append(
      title,
      $$(DialogSectionComponent, { label: this.getLabel('supplementary-file-upload-label') }).append(
        $$(SupplementaryFileUploadComponent),
      ),
      $$(DialogSectionComponent, { label: this.getLabel('supplementary-file-link-label') }).append(urlInput),
    );
