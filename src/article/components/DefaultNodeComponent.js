...
export default class DefaultNodeComponent extends Component {
  ...
  render($$) {
    ...
    for (let name of visiblePropNames) {
      ...
      el.append(this._renderProperty($$, name, value, nodeIssues));
    }
    ...
    if (hasHiddenProps) {
      ...
      if (showAllFields) {
        controlEl
          .append($$(FontAwesomeIcon, { icon: 'fa-chevron-up' }).addClass('se-icon'), this.getLabel('show-less-fields'))
  ...
  _renderProperty($$, name, value, nodeIssues) {
    ...
    if (PropertyEditor) {
      ...
      return $$(FormRowComponent, {
        ...
        .addClass(`sm-${name}`)
        .append($$(PropertyEditor, editorProps).ref(name));
    }
  }
  ...
  _renderHeader($$) {
    ...
    if (ModelPreviewComponent) {
      header.append($$(ModelPreviewComponent, { node }));
    }
    return header;
  }
