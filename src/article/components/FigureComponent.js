  ...
  */
  ...
  _renderCarousel($$, panels) {
    if (panels.length === 1) {
      ...
      return $$('div')
        .addClass('se-carousel')
        .append(
          this._renderNavigation($$),
          $$('div')
            .addClass('se-current-panel')
            .append(this._renderCurrentPanel($$)),
        );
    }
  }
  ...
  _renderNavigation($$) {
    ...
    const leftControl = $$('div')
      .addClass('se-control sm-previous')
      .append(this._renderIcon($$, 'left-control'));
    if (currentIndex > 1) {
    ...
    const rightControl = $$('div')
      .addClass('se-control sm-next')
      .append(this._renderIcon($$, 'right-control'));
    ...
    return $$('div')
      .addClass('se-navigation')
      .append(
        $$('div')
          .addClass('se-current-position')
          .append(currentPosition),
        $$('div')
          .addClass('se-controls')
          .append(leftControl, rightControl),
      );
  }
  ...
  _renderIcon($$, iconName) {
    return $$('div')
      .addClass('se-icon')
      .append(this.context.iconProvider.renderIcon($$, iconName));
  }
}
