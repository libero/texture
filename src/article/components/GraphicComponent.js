...
export default class GraphicComponent extends NodeComponent {
  render($$) {
    ...
    if (this.state.errored) {
      let errorEl = $$(this.tagName)
        .addClass('se-error')
        .append(this.context.iconProvider.renderIcon($$, 'graphic-load-error').addClass('se-icon'));
  ...
  _renderError($$, errorEl) {
    errorEl.append(this.getLabel('graphic-load-error'));
  }
