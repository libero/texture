...
export default class ManuscriptEditor extends EditorPanel {
  ...
  render($$) {
    let el = $$('div')
      .addClass('sc-manuscript-editor')
      // sharing styles with sc-article-reader
      .addClass('sc-manuscript-view');
    el.append(this._renderMainSection($$), this._renderContextPane($$));
  ...
  _renderMainSection($$) {
    ...
    if (appState.workflowId) {
      mainSection.append(this._renderWorkflow($$, appState.workflowId));
    }
  ...
  _renderTOCPane($$) {
    ...
    el.append(
      $$('div')
        .addClass('se-context-pane-content')
        .append($$(ManuscriptTOC, { model: this._model })),
    );
    return el;
  }
  ...
  _renderContextPane($$) {
    ...
    if (this.props.contextComponent) {
      ...
      el.append(
        $$('div')
          .addClass('se-context-pane-content')
          .append(this.props.contextComponent),
      );
      return el;
    }
  }
