...
export default class QueryComponent extends Component {
  render($$) {
    ...
    if (this.props.loading) {
      btnEl.append(this._renderIcon($$, 'input-loading'));
    } else if (this.props.errors) {
      btnEl.append(this._renderIcon($$, 'input-error'));
    } else {
      btnEl.append(this.getLabel(this.props.actionLabel)).on('click', this._onQuery);
    }
  ...
  _renderIcon($$, icon) {
    return $$('div')
      .addClass('se-icon')
      .append(this.context.iconProvider.renderIcon($$, icon));
  }
