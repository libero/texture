...
export default class TableComponent extends CustomSurface {
  ...
  _renderUnclickableOverlays($$) {
    ...
    el.append(this._renderSelectionOverlay($$));
    el.append(this.props.unclickableOverlays);
    return el;
  }
  ...
  _renderSelectionOverlay($$) {
    ...
    el.append(
      $$('div')
        .addClass('se-selection-anchor')
        .ref('selAnchor')
        .css('visibility', 'hidden'),
      $$('div')
        .addClass('se-selection-range')
        .ref('selRange')
        .css('visibility', 'hidden'),
    );
    return el;
    ...
    if (isRightButton) {
      ...
      if (targetInfo.type === 'cell') {
        ...
        if (_selData && targetCell) {
          let { startRow, startCol, endRow, endCol } = getCellRange(
            this.props.node,
            _selData.anchorCellId,
            _selData.focusCellId,
          );
          _needSetSelection =
            targetCell.colIdx < startCol ||
            targetCell.colIdx > endCol ||
            targetCell.rowIdx < startRow ||
            targetCell.rowIdx > endRow;
        }
  ...
  */
  ...
  _getStylesForRectangle(rect) {
    ...
    if (rect) {
      ...
      if (isFinite(rect.top) && isFinite(rect.left) && isFinite(rect.width) && isFinite(rect.height)) {
