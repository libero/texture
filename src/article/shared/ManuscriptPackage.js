...
import { BasePackage, EditorBasePackage, ModelComponentPackage, FindAndReplacePackage } from '../../kit';
...
import {
  ...
  AddEntityCommand,
  AddFigureMetadataFieldCommand,
  AddFigurePanelCommand,
  AddReferenceCommand,
  MoveMetadataFieldCommand,
...
import {
  ...
  ManuscriptTOC,
  InsertFigurePanelTool,
  DownloadSupplementaryFileTool,
  InsertFigureTool,
  InsertInlineGraphicTool,
  OpenFigurePanelImageTool,
  ReplaceFigurePanelTool,
  ReplaceSupplementaryFileTool,
  InsertTableTool,
  ManuscriptEditor,
} from '../components';
...
export default {
  ...
  configure(config) {
    ...
    config.addIcon('edit-section', { fontawesome: 'fa-edit' });
    ...
    config.addToolPanel('main-overlay', [
      {
        ...
        style: 'minimal',
        hideDisabled: true,
        items: [{ type: 'command-group', name: 'prompt' }],
      },
    ]);
    ...
    config.addToolPanel('workflow', [
      {
        name: 'workflow',
        type: 'group',
        items: [{ type: 'command-group', name: 'workflows' }],
      },
    ]);
    ...
    registerCollectionCommand(config, 'funder', ['metadata', 'funders'], {
      keyboardShortcut: 'CommandOrControl+Alt+Y',
    });
    registerCollectionCommand(config, 'editor', ['metadata', 'editors'], {
      keyboardShortcut: 'CommandOrControl+Alt+E',
      nodeType: 'person',
    });
    registerCollectionCommand(config, 'group', ['metadata', 'groups'], { keyboardShortcut: 'CommandOrControl+Alt+G' });
    registerCollectionCommand(config, 'keyword', ['metadata', 'keywords'], {
      keyboardShortcut: 'CommandOrControl+Alt+K',
    });
    registerCollectionCommand(config, 'affiliation', ['metadata', 'affiliations'], {
      keyboardShortcut: 'CommandOrControl+Alt+O',
    });
