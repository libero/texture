...
export default class AddReferenceWorkflow extends Component {
  ...
  render($$) {
    ...
    const title = $$('div')
      .addClass('se-title')
      .append(this.getLabel('add-reference-title'));

    const refTypesButtons = $$('ul').addClass('se-reftypes-list');
    INTERNAL_BIBR_TYPES.forEach(item => {
      refTypesButtons.append(
        $$('li')
          .addClass('se-type sm-' + item)
          .append(this.getLabel(item))
          .on('click', this._onAdd.bind(this, item)),
      );
    });
    const manualAddEl = $$('div')
      .addClass('se-manual-add')
      .append(refTypesButtons);

    el.append(
      title,
      $$(DialogSectionComponent, { label: this.getLabel('fetch-datacite') }).append($$(DOIInputComponent)),
      ...
      $$(DialogSectionComponent, { label: this.getLabel('add-ref-manually') }).append(manualAddEl),
    );
